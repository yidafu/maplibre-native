...
class ForwardingRendererObserver;
...
private:
    ...
    std::atomic<bool> renderingPaused{false};
    MapObserver::RenderMode renderingMode = MapObserver::RenderMode::Full;
    bool needsRender = false;
    
    // 🎯 新架构：渲染线程（替代 RunLoop thread）
    std::unique_ptr<HarmonyRenderThread> renderThread_;
    
    // Map RunLoop 引用（用于 ForwardingObserver）
    util::RunLoop* mapRunLoop_ = nullptr;
    ...
    // ForwardingObserver（转发渲染回调到 Map RunLoop）
    std::shared_ptr<ForwardingRendererObserver> forwardingObserver_;
