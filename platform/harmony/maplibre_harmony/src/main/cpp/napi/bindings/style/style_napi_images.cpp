...
napi_value StyleNAPI::AddImageAsync(napi_env env, napi_callback_info info) {
    ...
    if (argc < 1) {
        napi_throw_error(env, nullptr, "AddImageAsync requires at least 1 argument");
        return nullptr;
    }
...
napi_value StyleNAPI::AddImagesAsync(napi_env env, napi_callback_info info) {
    ...
    if (argc < 1) {
        napi_throw_error(env, nullptr, "AddImagesAsync requires an array of Image objects");
        return nullptr;
    }
    ...
    try {
        ...
        for (uint32_t i = 0; i < arrayLength; ++i) {
            ...
            napi_get_element(env, args[0], i, &imageValue);
