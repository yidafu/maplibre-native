...
// GetProcAddress implementation for Harmony platform
gl::ProcAddress GetProcAddress(const char* name) {
    gl::ProcAddress address = reinterpret_cast<gl::ProcAddress>(eglGetProcAddress(name));
    if (!address) {
        std::stringstream message;
        message << "Failed to load OpenGL function: " << name;
        Log::Warning(Event::OpenGL, message.str());
    }
    return address;
}
